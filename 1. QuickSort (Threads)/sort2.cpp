...
pthread_cond_t taskcomplete_cond;
...
void* ThreadProc(void* argv)
{
  ...
  pthread_cond_signal(&taskcomplete_cond);
  ...
  while((arg->first) <= (arg->last))
  {
      ...
      //Найден элемент меньше для вставки//wait
      ...
      //Позиция, с которой начнем проверять - первый эл-т какого-либо подмассива
...
int main(int argc, char** argv)
{
  ...
  pthread_cond_init(&taskcomplete_cond, NULL);
  ...
  {
      pthread_cond_wait(&taskcomplete_cond, &mutex);
  }
  ...
  for(h = 0; h < count; h++)
  {
      ...
      pthread_cond_broadcast(&insertion_finished_cond);
  ...
  pthread_cond_destroy (&taskcomplete_cond);
